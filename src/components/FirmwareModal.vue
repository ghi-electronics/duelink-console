<template>
  <Modal :open="open" max-width-class="sm:max-w-xl">
    <template #title> Update Firmware Steps </template>

    <ol class="mb-4 ol-reset space-y-2 leading-loose">
      <li>Connect your board to the computer.</li>
      <li>
        On your board, hold down the <kbd>A</kbd> button while pressing and
        releasing the <kbd>RESET</kbd> button once.
        <span class="font-semibold"
          >Keep holding the <kbd>A</kbd> button down,</span
        >
        wait for a second, and then release it.
      ...
      <li>
        ...
        <em>GHI Bootloader Interface</em>.
      </li>
      <li>
      ...
      <li>
        Once <span class="font-semibold">Loading is complete</span>, press and
        release the <kbd>RESET</kbd> button.
      </li>
      ...
      <div class="flex space-x-2">
        <template v-if="isConnected">
          <template v-if="state === 'idle'">
            <Button
              :disabled="
                !firmware || !availableFirmware[firmware].image[version]
              "
...
const availableVersions = computed(() => {
  const firmwareData = props.availableFirmware[firmware.value];
  const versions = firmwareData?.versions ?? {}; // Ensure versions is an object

  return Object.fromEntries(
    Object.entries(versions).sort(([keyA], [keyB]) => keyA.localeCompare(keyB))
  );
});
...
watch(
  () => firmware.value,
  () => {
    version.value = ""; // Reset the selected version
    console.log("Firmware changed, resetting version.");
  }
);
...
watch(
  ...
  async (key) => {
    ...
    if (response.ok) {
      ...
      props.availableFirmware[firmware.value].image[version.value] = data;
...
async function writeFirmware() {
  ...
  error.value = null;
  percent.value = 0;
  if (!props.availableFirmware[firmware.value].image[version.value]) {
    error.value =
  ...
  await ghiLoader.flash(
    props.availableFirmware[firmware.value].image[version.value],
