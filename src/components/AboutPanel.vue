<template>
  ...
  <div
    v-if="
      ...
      deviceFirmwareVersion !== latestFirmwareVersion
    "
    class="px-4 py-2 font-medium bg-yellow-200 dark:text-zinc-900"
  >
    <i class="fas fa-fw fa-triangle-exclamation mr-1"></i> Please update to the
    latest firmware.
  </div>
...
const deviceChar = computed(() => {
  if (props.version) {
    return props.version.substring(
      props.version.length - 1,
      props.version.length
    );
  }
  return null;
});
...
const firmware = computed(() =>
  Object.values(props.availableFirmware).find((firmware) =>
    (firmware?.boards || []).find((board) => board.id === deviceChar.value)
  )
);

const device = computed(() => {
  if (firmware.value) {
    const board = firmware.value.boards.find(
      (board) => board.id === deviceChar.value
    );
    if (board) {
      return board.name;
    }
...
const latestFirmwareVersion = computed(() => {
  if (Object.values(props.availableFirmware).length) {
    ...
    const maxId = Math.max(
      Object.keys({ ...props.availableFirmware[key].versions })
    );
    const version = props.availableFirmware[key].versions[maxId];
    return `${version.name}`;
  }
  // if (firmware.value) {
