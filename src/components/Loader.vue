    ...
    <div>
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="max-w-3xl mx-auto space-y-8">
                ...
                <div>
                    <textarea v-model="code" class="w-full" rows="6"></textarea>
                    <div>{{ code }}</div>
                    <button v-if="isConnected" @click="sendCode" type="button" class="inline-flex items-center px-4 py-2 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-gray-600 hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
                        Send Code
                    </button>
                </div>
...
export default {
    ...
    data() {
        return {
            availableFirmware: {
                ...
                'BrainPad_Experimental_0_0_7': {
                    title: 'BrainPad_Experimental (v0.0.7)',
                    url: 'firmware/Brainpad_experimental_007.ghi',
                    isGlb: false,
                    image: null,
                },
            },
    ...
    methods: {
        ...
        async sendCode() {
            //await this.ghiLoader.sendAndExpect('Esc', 'Esc');
            await this.ghiLoader.sendAndExpect('$', '$');
            const lines = this.code.split("\n");
            for (const line of lines) {
                await this.ghiLoader.sendAndExpect(line.trim(), '$');
            }
            await this.ghiLoader.sendAndExpect('run', '$');
        },
