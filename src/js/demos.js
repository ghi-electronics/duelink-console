export default [
    {
        label: 'Hello World',
        code: [
            '# Print to output',
            'Print("Hello World")',
            '# Print to display, if available',
            '# Will cause an error when using BrainPad Edge (SC007)',
            'LcdClear(0)',
            'LcdText("Hello World!", 1, 30, 30)',
            'LcdShow()',
        ],
    },
    {
        label: 'Blink LED',
        code: [
            '@Loop',
            '  DWrite(108, 1) : Wait(250)',
            '  DWrite(108, 0) : Wait(250)',
            'Goto Loop',
        ],
    },
    {
        label: 'Fade LED',
        code: [
            '@Loop',
            '  For i=0 to 1000 Step 100',
            '    AWrite("L", i)',
            '    Wait(100)',
            '  Next',
            '  For i=1000 to 0 Step -100',
            '    AWrite("L", i)',
            '    Wait(100)',
            '  Next',
            'Goto Loop',
        ],
    },
    {
        label: 'Read Button',
        code: [
            'BtnEnable("A", 1)',
            '@Loop',
            '  x=BtnDown("A")',
            '  If x=1',
            '    PrintLn("Button A")',
            '  End',
            '  Wait(200)',
            'Goto Loop',
        ],
    },
    {
        label: 'Cap Touch Sense',
        code: [
            '# This sample reads pin 0 as a capacitive touch.',
            '# Touch with your finger to detect',
            '# BrainPad already has the required R/C',
            '# Other pins need R/C to work',
            '# See https://duelink.com/software/due-script/corelib/touch.html',
            '@Loop',
            '  a=TouchRead(0)',
            '  If a>0:PrintLn("pin 0"):End',
            '  Wait(100)',
            'Goto Loop',
        ],
    },
    {
        label: 'For Loop',
        code: [
            'For i=0 to 10',
            '  Print(i)',
            'Next',
        ],
    },
    {
        label: 'If Statement',
        code: [
            'If x=1',
            '  PrintLn("one")',
            'Else',
            '  PrintLn("not one")',
            'End',
        ],
    },
    {
        label: 'Subroutine',
        code: [
            '@Go',
            '  Print("I am going!")',
            'Return',
            '# You can now execute Go() from the code',
            '# And also can execute it from immediate mode',
            '# Enter Go() in the immediate window',
        ],
    },
    {
        label: 'BrainPad Pulse',
        code: [
            '# BrainPad Pulse - LCD Bouncing ball',
            'x=64 # Ball X location',
            'y=32 # Ball Y location',
            'r=5  # Ball radius',
            'a=3  # Ball X Speed',
            'b=2  # Ball Y Speed',
            '@Loop',
            '  LCDClear(0)        # Clear the screen',
            '  LCDCircle(1,x,y,r) # Draw the ball',
            '  LCDShow()          # Update the screen',
            '  x=x+a:y=y+b        # Move the ball',
            '  # Check if the ball hit one of the edges',
            '  # If it did, then reverse the direction and make a sound',
            '  If x<r || x>=(128-r):a=-a:sound(500,16,100):End',
            '  If y<r || y>=(64-r):b=-b:sound(2000,16,100):End',
            'Goto Loop',
        ],
    },
    {
        label: 'More Samples',
        url: 'https://duelink.com/software/due-script/samples/samples.html',
    },
];
