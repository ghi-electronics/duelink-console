...
class Xmodem extends EventEmitter {
    ...
    async send(reader, writer, dataToSend, progressfn) {
        ...
        this.log.info("Bytes to send : " + dataToSend.length);

        await this.sendLoop(reader, writer, new Uint8Array(dataToSend), progressfn);
    }
    ...
    async sendLoop(reader, writer, dataToSend, progressfn) {
        ...
        while (dataToSend.length > 0) {
            for (i = 0; i < this.block_size; i++) {
                current_block[i] = dataToSend[i] === undefined ? this.FILLER : dataToSend[i];
            }
        ...
        while (!sent_eof) {
            try {
                ...
                if (value) {
                    ...
                    if (command === this.CRC_MODE && blockNumber !== this.XMODEM_START_BLOCK) {
                        // still eating 'C' characters
                        continue
                    }
                    else if (command === this.CRC_MODE && blockNumber === this.XMODEM_START_BLOCK) {
                        ...
                        if (packagedBuffer.length > blockNumber) {
                            ...
                            this.sendBlock(writer, blockNumber, packagedBuffer[blockNumber], this.XMODEM_OP_MODE);
                            this.emit('status', { action: 'send', signal: 'STX', block: blockNumber });
                            blockNumber++;
                        }
                    }
                    else if (command === this.NAK && blockNumber === this.XMODEM_START_BLOCK) {
                        this.log.info("[SEND] - received NAK byte for standard checksum transfer!");
                     ...
                     */
                    else if (command === this.ACK && blockNumber > this.XMODEM_START_BLOCK) {
                        ...
                        if (blockNumber === packagedBuffer.length && sent_eof) {
                            this.log.info('[SEND] - Resending EOT, because receiver responded with NAK.');
                            this.emit('status', { action: 'send', signal: 'EOT' });
                            writer.write(Buffer.from[this.EOT]);
                        } else {
                            this.log.info('[SEND] - Packet corruption detected, resending previous block.');
...
//    var sendBuffer = Buffer.concat([Buffer.from([this.SOH]),
        var sendBuffer = Buffer.concat([Buffer.from([this.STX]), // use STX to indicate Xmodem-1K protocol
            Buffer.from([blockNr]),
        ...
        if (cmd === this.STX) {
            if ((block + block_check) === 0xFF) {
                ...
                if (block === (blockNr % 0x100)) {
                    ...
                    this.log.error('ERROR: Synch issue! Received: ' + block + ' Expected: ' + blockNr);
                    return;
                }
            }
            else {
            ...
            if (current_block.length === block_size) {
                ...
                this.log.error('ERROR: Received block size did not match the expected size. Received: ' + current_block.length + ' | Expected: ' + block_size);
                socket.write(Buffer.from([this.NAK]));
                return;
            }
        }
