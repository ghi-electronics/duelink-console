<template>
    <div id="progress-bar">
        <div :ref="(el) => $refs.progress = el"
            class="h-1 bg-sky-500 dark:bg-lime-500 transition duration-1000 ease-linear opacity-0"></div>
    </div>
    <div class="min-h-screen flex flex-col space-y-0.5">
        <MenuBar v-model:theme="theme" @demo="demo" @update:theme="updateTippyTheme" @updateTippy="updateTippy" />
        <ToolBar :can-download="canDownload" :can-list="true" :can-load="true" :can-play="canPlay"
            :can-record="canRecord" :can-stop="canStop" :can-text-size-plus="textSize < 40"
            :can-text-size-minus="textSize > 12" :is-busy="webSerial.isBusy.value"
            :is-connected="webSerial.isConnected.value" :is-talking="webSerial.isTalking.value" :devAdd="sel_devaddr"
            @connect="do_connect" @disconnect="webSerial.disconnect()" @download="downloadModal.start()"
            @play="webSerial.play()" @stop="webSerial.stop()" @record="sendRecordMode" @list="sendList" @load="onLoad"
            @text-size-plus="textSizePlus" @text-size-minus="textSizeMinus" @sel_cmd="sel_cmd_msgbox_show"
            @load_driver="load_driver" @load_sample="load_sample" />
                    ...
                    <div class="flex-1">
                        <div class="relative">
                            <div class="absolute right-0 inset-y-0 p-1 flex items-center justify-center rounded-r-md"
                                ...
                                <div :class="[directModeCode.length
                            ...
                            <input v-model="directModeCode" :ref="(el) => $refs.input = el"
                                placeholder="Code to run immediately..." type="text" @keyup.enter="sendDirectMode" />
                        </div>
                    </div>
                    <Button :disabled="disabled" data-tippy-content="Execute" @click.native="sendDirectMode">
                        <i class="fas fa-fw fa-arrow-right"></i>
                    </Button>
                    <Button :disabled="disabled" data-tippy-content="Stop" @click.native="sendEscape">
            ...
            <div class="dialog">
                ...
                <div class="dialog-body">
                    <p v-if="sel_devaddr === 1">
                        This feature only works on modules loaded with either DUELink official firmware or MicroBlocks
                        firmware. It will completely erase the device and put it in DFU (Device Firmware Update)
                        mode.<br><br>Note: The following <strong>Connect</strong> window will be empty if the device is
                        already in DFU mode or it has an uncompatible firmware, see <a target="_blank"
                            href="https://www.duelink.com/docs/loader">Loader page</a>.
                    </p>
                    <p v-else>
                ...
                <div class="dialog-buttons">
                    <button v-if="sel_devaddr === 1" class="yes" @click="do_eraseall_dms_pre_yes">
                        Continue
                    </button>
                    <button :class="sel_devaddr === 1 ? 'no' : 'yes'" @click="do_eraseall_dms_pre_no">
                        Abort
                    </button>
        ...
        <div v-if="sel_cmd_msgbox" class="overlay">
            <div class="dialog">
                ...
                <div class="dialog-body">
                    ...
                    <label for="device-number" style="display: block; margin-top: 10px;">
                        Module Address:
                        <input id="device-number" type="number" min="1" max="254" v-model.number="sel_devaddr"
                            placeholder="Enter device number" style="width: 70px; height: 26px;margin-top: 1px;"
                            @blur="onDeviceNumberBlur" />
                    </label>
                </div>
        ...
        <div v-if="update_driver_msgbox_confirm_final" class="overlay">
            <div class="dialog">
                ...
                <div class="dialog-body">
                    ...
                    <br>
                    <p>Do you want to erase all scripts and load <a target="_blank"
            ...
            <div v-if="connect_msgbox_progress" class="overlay">
                <div class="dialog" style="width: 25vw;">
                    <div class="dialog-title" :class="{ 'dialog-title-success': percent_tmp === 100 }">
                        ...
                        <!-- Show icon only while writing -->
                        <i v-if="percent_tmp < 100" class="fas fa-exclamation-triangle" style="margin-right: 8px;"></i>
                        {{ percent_tmp < 100 ? "Please wait..." : "Connected" }} </div>
                ...
                <div v-if="eraseall_dms_msgbox_finished" class="overlay">
                    <div class="dialog">
                        ...
                        <div class="dialog-body">
                            <p>"Erase All" operation completed. You are in DFU Mode now!<br><br>Click the Connect button
                                on the next window.</p>
                        </div>
                ...
                <div v-if="progressbar_standard" class="overlay">
                    <div class="dialog" style="width: 25vw;">
                        <div class="dialog-title" :class="{ 'dialog-title-success': percent_tmp === 100 }">
                            <!--<i class="fas fa-exclamation-triangle" style="color: yellow; margin-right: 8px;"></i>-->
                            <!-- Show icon only while writing -->
                            <i v-if="percent_tmp < 100" class="fas fa-exclamation-triangle"
                ...
                <Modal :open="downloadModal.open">
                    ...
                    <div>
                        ...
                        <input v-model="filename" :ref="(el) => $refs.filename = el" class="mt-2" id="filename"
                            type="text" />
                    </div>
                    <template #buttons>
                        <div class="flex space-x-2">
                            <Button :disabled="filename.length === 0" class="w-full"
                                @click.native="downloadModal.download()">
                                Download
                            </Button>
                ...
                <FirmwareModal :available-firmware="availableFirmware" :open="firmwareModal.open"
                    @close="firmwareModal.open = false" />

                <DFUModal :available-dfu="availableDfu" :open="dfuModal.open" @close="dfuModal.open = false"
                    @eraseall_dms_dfumodal="eraseall_dms_show_confirm_pre" />

                <ListAllModal :code="listAllModal.code" :language="language" :open="listAllModal.open"
                    :text-size="textSize" :theme="theme" @close="listAllModal.open = false" />

                <NewAllModal :open="newAllModal.open" @close="newAllModal.open = false"
