<template>
  ...
  <div class="min-h-screen flex flex-col space-y-0.5">
    ...
    />
    <div
      class="flex-1 flex flex-col space-y-0.5 sm:space-y-0 sm:flex-row sm:space-x-0.5"
    >
    ...
    <template #buttons>
      <div class="flex space-x-2">
        ...
        <Button
          class="w-full"
          type="secondary"
          @click.native="alreadyHasCodeModal.no()"
        >
          No
        </Button>
      </div>
    ...
    <template #buttons>
      <div class="flex space-x-2">
        ...
        <Button
          class="w-full"
          type="secondary"
          @click.native="downloadModal.cancel()"
        >
          Cancel
        </Button>
      </div>
...
const canPlay = computed(
  () =>
    ...
    recordModeCode.value === lastRecordModeCode.value &&
    !webSerial.isPlaying.value
);
const canRecord = computed(
  () =>
    recordModeCode.value.length > 0 &&
    recordModeCode.value !== lastRecordModeCode.value
);
const canStop = computed(() => !webSerial.isStopped.value);
const disabled = computed(
  () =>
    !webSerial.isConnected.value ||
    webSerial.isBusy.value ||
    webSerial.isTalking.value
);
...
async function loadFirmware() {
  try {
    ...
    Object.keys(jsonData).forEach((key) => {
      ...
      availableFirmware[key].image = {};

      for (let version in jsonData[key].versions) {
        availableFirmware[key].image[version] = null;
      }
    });
  } catch (error) {
    console.log(error);
...
function updateTippyTheme() {
  tippyInstances.forEach((instance) =>
    instance.setProps({ theme: theme.value })
  );
}
